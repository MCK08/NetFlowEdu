...
describe("mergeQuestionSignals", () => {
  it("sums counts for a question present in both maps", () => {
    const live = new Map([["q1", { everAttemptedCount: 2, struggledCount: 1, mostRecentReviewedAt: 100 }]]);
    const historical = new Map([["q1", { everAttemptedCount: 3, struggledCount: 2, mostRecentReviewedAt: 50 }]]);
    const merged = mergeQuestionSignals(live, historical);
    expect(merged.get("q1")).toEqual({ everAttemptedCount: 5, struggledCount: 3, mostRecentReviewedAt: 100 });
  });

  it("keeps a question present only in live signals unchanged", () => {
    const live = new Map([["q1", { everAttemptedCount: 1, struggledCount: 0, mostRecentReviewedAt: null }]]);
    const merged = mergeQuestionSignals(live, new Map());
    expect(merged.get("q1")).toEqual({ everAttemptedCount: 1, struggledCount: 0, mostRecentReviewedAt: null });
  });

  it("keeps a question present only in historical signals unchanged", () => {
    const historical = new Map([["q1", { everAttemptedCount: 4, struggledCount: 4, mostRecentReviewedAt: 10 }]]);
    const merged = mergeQuestionSignals(new Map(), historical);
    expect(merged.get("q1")).toEqual({ everAttemptedCount: 4, struggledCount: 4, mostRecentReviewedAt: 10 });
  });

  it("never mutates either input map", () => {
    const live = new Map([["q1", { everAttemptedCount: 1, struggledCount: 0, mostRecentReviewedAt: null }]]);
    const historical = new Map([["q1", { everAttemptedCount: 1, struggledCount: 1, mostRecentReviewedAt: 5 }]]);
    mergeQuestionSignals(live, historical);
    expect(live.get("q1")).toEqual({ everAttemptedCount: 1, struggledCount: 0, mostRecentReviewedAt: null });
    expect(historical.get("q1")).toEqual({ everAttemptedCount: 1, struggledCount: 1, mostRecentReviewedAt: 5 });
  });

  it("is deterministic", () => {
    const live = new Map([["q1", { everAttemptedCount: 1, struggledCount: 0, mostRecentReviewedAt: null }]]);
    const historical = new Map([["q2", { everAttemptedCount: 1, struggledCount: 1, mostRecentReviewedAt: 5 }]]);
